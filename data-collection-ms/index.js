...
const SEARCH_KEYWORDS = [ // This could be maintained in the db, if list grows or changes frequently.
  'drone',
  'uav',
...
async function fetchHeadlines() {
  ...
  const requests = SEARCH_KEYWORDS.map(keyword => newsApi.v2.topHeadlines({
    q: keyword
  }));
  ...
  let headlinesToSave = responses.reduce((headlinesMap, res, index) => {
    if (res.status === 'fulfilled') {
      console.log(`Headlines for keyword ${SEARCH_KEYWORDS[index]} fetched successfully. Retrieved ${res.value?.articles?.length} articles.`);
      ...
      console.error(`Failed to fetch headlines for keyword ${SEARCH_KEYWORDS[index]}`, res);
    }
  }, {});
  ...
  if (headlinesToSave.length) {
    try {
      ...
      console.error(`Failed to store headlines to db - next interval will reattempt`, err);
    }
  } else { 
